...
- **复用度（复审）：** high —— 三步设计管线完整（example.sh 参数详解）、内置 inpainting 权重、标准 motif-scaffolding 基准集与 benchmark notebook、免依赖 Colab；环境依赖 ESMFold/openfold 较重但资产齐全
- **许可证（复审）：** MIT
- **能力（复审）：** inference、training_pipeline、benchmark、data_loader
