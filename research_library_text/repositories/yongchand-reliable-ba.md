...
- **复用度（复审）：** high —— train→infer→analyze 全管线完整，附带小样本示例数据集与一键 demo 脚本，训练/验证/测试 CSV 内置；只需自备引擎打分与嵌入即可扩展
- **许可证（复审）：** MIT
- **能力（复审）：** training_pipeline、inference、data_loader、visualization、benchmark
