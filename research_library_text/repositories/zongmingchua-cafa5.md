...
- **复用度（复审）：** high —— pip 安装 + Kaggle 数据（含预计算嵌入，可跳过嵌入再生）+ CLI 三命令完整训练预测合并流程，含 pytest 测试与详尽文档
- **许可证（复审）：** MIT
- **能力（复审）：** data_loader、training_pipeline、inference、benchmark、database、protocol
