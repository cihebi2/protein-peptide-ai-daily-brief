...
- **复用度（复审）：** medium —— 推理/demo 管线完整且有权重下载链接和 AKT2 示例数据，但训练代码明确未上传（README to-do 标注），且需自行拉取 scGPT 仓库与配置环境
- **许可证（复审）：** none（根目录无 LICENSE；hgraph/ 子目录携带上游 hierVAE 的 MIT）
- **能力（复审）：** inference、data_loader、visualization
