...
- **复用度（复审）：** medium —— 训练/采样/评测/docking 基线脚本完整（conda 环境文件齐备），但 CrossDocked 数据需按 TargetDiff 流程自备且无预训练权重
- **许可证（复审）：** none
- **能力（复审）：** data_loader、training_pipeline、inference、benchmark、protocol
