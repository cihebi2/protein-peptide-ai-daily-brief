...
- **复用度（复审）：** high —— cliff 生成管线完整（生成+划分+相似度矩阵归一化），且仓库自带 GRAMPA S.aureus 数据与 BLOSUM62/Tanimoto 矩阵，conda 环境文件齐备，数据在本地即可复跑
- **许可证（复审）：** none
- **能力（复审）：** data_loader、benchmark、database、protocol
